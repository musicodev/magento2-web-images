...
namespace MusicoDev\WebImages\Model\Image\Adapter;
...
class Gd2 extends AbstractAdapter
{
	...
	public function open($filename)
	{
		...
		if ($this->helper->isVectorImage($filename)) {
			$this->_isVector = true;
			$dimensions = $this->helper->getVectorImageDimensions($filename);
			$this->_imageSrcWidth = $dimensions['width'];
			$this->_imageSrcHeight = $dimensions['height'];
			$this->_fileMimeType = 'image/svg+xml';
			return;
		}
	...
	{
		$file = file_get_contents((string) $filename, false, null, 0, 25);

		if ($file[15] === 'L') {
			return (bool) (!!(ord($file[24]) & 0x10));
		} elseif ($file[15] === 'X') {
			return (bool) (!!(ord($file[20]) & 0x10));
		}
	...
	/**
	 * Gives true for a AVIF with alpha, false otherwise
	 *
	 * @param string $filename
	 * @return bool
	 */
	...
	{
		$file = file_get_contents((string) $filename, false, null, 0, 100);

		if (strlen($file) < 12 || substr($file, 4, 4) !== 'ftyp') {
			return false;
		}

		$pattern = '/\x00\x00\x00\x00\x61\x6C\x70\x68/';
	...
	public function watermark($imagePath, $positionX = 0, $positionY = 0, $opacity = 30, $tile = false)
	{
		...
		list(
			...
			$watermarkSrcHeight,
			$watermarkFileType,
		) = $this->_getImageOptions($imagePath);
		$this->_getFileAttributes();
	...
	{
		if ($this->_isVector) {
			$dimensions = $this->helper->getVectorImageDimensions($this->_fileName);
			$this->_imageSrcWidth = $dimensions['width'];
			$this->_imageSrcHeight = $dimensions['height'];
			return;
		}
